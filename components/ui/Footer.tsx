import Link from 'next/link'
import { useTranslations, useLocale } from 'next-intl'
import Logo from './Logo'
import { clientData } from '@/data/client-info'
import FooterCookieButton from './FooterCookieButton'

export default function Footer() {
  const t = useTranslations()
  const locale = useLocale()

  const navLinks = [
    { key: 'espaco', href: '#espaco' },
    { key: 'fado', href: '#fado' },
    { key: 'menu', href: '#menu' },
    { key: 'reservas', href: '#reservas' },
    { key: 'testemunhos', href: '#testemunhos' },
    { key: 'contactos', href: '#contactos' },
  ]

  const legalLinks = [
    { key: 'privacy', href: `/${locale}/privacy-policy` },
    { key: 'terms', href: `/${locale}/terms-of-service` },
    { key: 'cookies', href: `/${locale}/cookie-policy` },
    { key: 'cancellation', href: `/${locale}/cancellation-policy` },
  ]

  return (
    <footer className="bg-charcoal text-white/80" role="contentinfo">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">

          {/* Brand */}
          <div className="lg:col-span-1">
            <Logo className="h-12 mb-4" isDark />
            <p className="font-body text-white/60 text-sm leading-relaxed mt-4">
              {t('footer.tagline')}
            </p>
            {/* Social */}
            <div className="flex gap-3 mt-6">
              <a
                href={clientData.social.facebook}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Arcadas do Fado no Facebook"
                className="w-9 h-9 flex items-center justify-center rounded-full bg-white/10 hover:bg-rust transition-colors duration-200"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href={clientData.social.instagram}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Arcadas do Fado no Instagram"
                className="w-9 h-9 flex items-center justify-center rounded-full bg-white/10 hover:bg-rust transition-colors duration-200"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Quick links */}
          <div>
            <h3 className="font-sans text-xs font-semibold tracking-[0.15em] uppercase text-white/50 mb-5">
              {t('footer.quickLinks')}
            </h3>
            <ul className="space-y-3">
              {navLinks.map(({ key, href }) => (
                <li key={key}>
                  <a
                    href={href}
                    className="font-body text-white/70 hover:text-white transition-colors duration-200 text-sm"
                  >
                    {t(`nav.${key}` as Parameters<typeof t>[0])}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="font-sans text-xs font-semibold tracking-[0.15em] uppercase text-white/50 mb-5">
              {t('footer.contact')}
            </h3>
            <ul className="space-y-3">
              <li className="font-body text-white/70 text-sm leading-relaxed">
                {clientData.contact.address}
              </li>
              <li>
                <a
                  href={`tel:${clientData.contact.phoneClean}`}
                  className="font-body text-white/70 hover:text-white transition-colors duration-200 text-sm"
                >
                  {clientData.contact.phone}
                </a>
              </li>
              <li>
                <a
                  href={`https://wa.me/${clientData.contact.mobileClean}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="font-body text-white/70 hover:text-white transition-colors duration-200 text-sm"
                >
                  {clientData.contact.mobile}
                </a>
              </li>
              <li>
                <a
                  href={`mailto:${clientData.contact.email}`}
                  className="font-body text-white/70 hover:text-white transition-colors duration-200 text-sm"
                >
                  {clientData.contact.email}
                </a>
              </li>
            </ul>
          </div>

          {/* Legal */}
          <div>
            <h3 className="font-sans text-xs font-semibold tracking-[0.15em] uppercase text-white/50 mb-5">
              {t('footer.legal')}
            </h3>
            <ul className="space-y-3">
              {legalLinks.map(({ key, href }) => (
                <li key={key}>
                  <Link
                    href={href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-body text-white/70 hover:text-white transition-colors duration-200 text-sm"
                  >
                    {t(`footer.${key}` as Parameters<typeof t>[0])}
                  </Link>
                </li>
              ))}
              <FooterCookieButton />
            </ul>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="font-sans text-white/40 text-xs text-center sm:text-left">
            {t('footer.copyright')}
          </p>
          <a
            href="https://re-evolution.pt"
            target="_blank"
            rel="noopener noreferrer"
            className="font-sans text-white/40 hover:text-white/70 text-xs transition-colors duration-200"
          >
            {t('footer.developer')}
          </a>
        </div>
      </div>
    </footer>
  )
}
